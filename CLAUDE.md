...
**The discovery worker owns the full background pipeline**: `discover → enrich → filter → index → score → auto-tailor → cleanup`. It runs every ~2h on the homelab and is the only thing that writes scores or auto-generates docs. The FastAPI backend is now read-mostly for the frontend (jobs list, stats, on-demand tailor/cover for jobs the user wants to handle manually).

## Auto-generation policy

After scoring, every job with `fit_score >= 9` that doesn't already have a tailored CV + cover letter gets one generated by the discovery worker. Capped at `AUTO_TAILOR_PER_USER` (default 5) per user per cycle to bound LLM cost. Lower-scored jobs (7–8) stay user-initiated via the manual Tailor / Cover buttons in the dashboard.
