...
STAGE_ORDER = ("discover", "enrich", "filter", "index", "score")
...
def _run_filter() -> dict:
    """Stage: Location filter — remove country-restricted remote jobs."""
    try:
        from applypilot.discovery.filter import run_location_filter
        result = run_location_filter()
        return {"status": "ok", **result}
    except Exception as e:
        log.error("Location filter failed: %s", e)
        return {"status": f"error: {e}"}
...
def _run_sequential(ordered: list[str], workers: int = 1,
...
def _run_streaming(ordered: list[str], workers: int = 1,
                   user_id: int | None = None) -> dict:
    ...
    for name in ordered:
        ...
        t = threading.Thread(
            ...
            args=(name, tracker, stop_event, workers, user_id),
            name=f"stage-{name}",
            daemon=True,
        )
        threads[name] = t
...
def run_pipeline(
    ...
    # Execute
    if stream:
        result = _run_streaming(ordered, workers=workers, user_id=user_id)
    else:
        result = _run_sequential(ordered, workers=workers, user_id=user_id)
