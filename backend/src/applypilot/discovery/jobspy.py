...
def _title_ok(title: str | None, include_any: list[str], exclude_any: list[str]) -> bool:
    """Check if a job title passes include/exclude keyword filtering."""
    if not title:
        return False
...
def store_jobspy_results(conn: sqlite3.Connection, df, source_label: str) -> tuple[int, int]:
    ...
    for _, row in df.iterrows():
        ...
        # Deduplicate: skip if same (title, company) already stored from another board
...
def _run_one_search(
    ...
    if other_sites:
        ...
        try:
            ...
            # Defensive fallback: bad country strings should not kill non-Glassdoor discovery.
            msg = str(e).lower()
            if "invalid country string" in msg and kwargs.get("country_indeed") != "worldwide":
                bad = kwargs.get("country_indeed")
                log.warning(
                    "[%s] Invalid country_indeed '%s'; retrying non-gd with 'worldwide'.",
                    label,
                    bad,
                )
                try:
    ...
    # Filter by title and location before storing
    before = len(df)
    ...
    if filtered:
        ...
        if title_filtered:
            msg += f" ({title_filtered} title"
            if loc_filtered:
                msg += f", {loc_filtered} location"
            msg += ")"
        elif loc_filtered:
            msg += f" ({loc_filtered} location)"
    log.info(msg)
...
def _full_crawl(
    ...
    include_titles = search_cfg.get("include_title_any", [])
    ...
    for s in searches:
        ...
        result = _run_one_search(
            ...
            accept_locs, reject_locs, include_titles, exclude_titles, glassdoor_map,
        )
        completed += 1
