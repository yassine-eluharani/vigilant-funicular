...
@router.post("/api/jobs/{encoded_url}/submit-prepared", response_model=StatusMutationResponse)
def submit_prepared(
    encoded_url: str, user: dict = Depends(get_current_user)
) -> StatusMutationResponse:
    """Approve a prepared application — flips apply_status to 'submitting'.

    The discovery worker on the homelab polls for `submitting` rows at the
    top of each cycle (and on its own short loop), re-runs the same handler
    with `submit=True`, and transitions the row to `applied` on success or
    `failed` with `apply_error` populated on failure.

    Only valid when the row is currently in `ready_to_submit` — anything
    else is a no-op so a stray double-click can't submit twice.
    """
    job_url = decode_url(encoded_url)
    conn = get_connection()
    row = conn.execute(
        "SELECT apply_status FROM user_jobs WHERE user_id = ? AND job_url = ?",
        (user["id"], job_url),
    ).fetchone()
    if not row:
        raise HTTPException(status_code=404, detail="job not found for user")
    current = row["apply_status"] if hasattr(row, "keys") else row[0]
    if current != "ready_to_submit":
        raise HTTPException(
            status_code=409,
            detail=f"job is not ready_to_submit (current status: {current!r})",
        )
    upsert_user_job(conn, user["id"], job_url, apply_status="submitting")
    _invalidate_stats(user["id"])
    return StatusMutationResponse(ok=True, status="submitting")


@router.post("/api/jobs/{encoded_url}/cancel-prepared", response_model=StatusMutationResponse)
def cancel_prepared(
    encoded_url: str, user: dict = Depends(get_current_user)
) -> StatusMutationResponse:
    """Discard a prepared application — clears apply_status so the worker
    re-prepares from scratch on the next cycle. Useful when the screenshot
    looks wrong and the user wants a fresh attempt rather than submitting.
    """
    job_url = decode_url(encoded_url)
    conn = get_connection()
    upsert_user_job(
        conn, user["id"], job_url,
        apply_status=None,
        apply_screenshot_url=None,
        apply_error=None,
    )
    _invalidate_stats(user["id"])
    return StatusMutationResponse(ok=True, status="cleared")


@router.post("/api/jobs/{encoded_url}/retry-apply", response_model=StatusMutationResponse)
def retry_apply(
    encoded_url: str, user: dict = Depends(get_current_user)
) -> StatusMutationResponse:
    """Re-queue a failed application — clears apply_status so the worker
    tries the prepare step again on its next cycle.
    """
    job_url = decode_url(encoded_url)
    conn = get_connection()
    upsert_user_job(
