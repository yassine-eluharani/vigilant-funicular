...
def load_employers() -> dict:
    ...
    Employers with enabled=false are excluded.
    """
    ...
    all_employers = data.get("employers", {})
    return {k: v for k, v in all_employers.items() if v.get("enabled", True)}
...
def store_results(conn: sqlite3.Connection, jobs: list[dict], employers: dict) -> tuple[int, int]:
    ...
    for job in jobs:
        ...
        # For Workday jobs, the employer IS the company
        if is_duplicate(conn, job.get("title"), site):
            existing += 1
            continue
